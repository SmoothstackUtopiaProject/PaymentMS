...
public class MOCKPaymentRepository {
  ...
  public static Optional<Payment> findById(Integer id) {
    List<Payment> paymentList = getTestPaymentList().stream()
      .filter(i -> i.getPaymentId().equals(id))
      .collect(Collectors.toList());
    return !paymentList.isEmpty()
      ? Optional.of(paymentList.get(0))
      : Optional.empty();
  } 

  public static Optional<Payment> findByBookingUuid(String bookingUuid) {
    List<Payment> paymentList = getTestPaymentList().stream()
  ...
  public static Optional<Payment> findByStripeUuid(String stripeUuid) {
    List<Payment> paymentList = getTestPaymentList().stream()
